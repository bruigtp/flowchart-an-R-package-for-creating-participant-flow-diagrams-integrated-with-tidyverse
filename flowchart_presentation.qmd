...
<hr style="width:2px; height:400px; background-color:#E0B046; border:none; margin-top: 30% !important; margin:0 auto; " />
...
<tbody>
  ...
  <tr>
   ...
   <td style="text-align:left;"> should percentages be calculated using the total number of rows of the dataset?
   </td>
  </tr>
  <tr>
   <td style="text-align:left;"> `sel_group=` </td>
   <td style="text-align:left;"> select the group in which to perform the filter/split.
   </td>
  </tr>
  <tr>
...
- There are a number of other arguments that you can check in the website vignettes:

<a href = "https://bruigtp.github.io/flowchart/articles/flowchart-customization.html">
![](WWW/vignette.png)
</a>

## `fc_theme()` {auto-animate="true"}

-   We can use the `fc_theme()` function to modify at once all arguments for all boxes:

```{r echo=TRUE, fig.width = 6, fig.height = 5, out.width = "600"}
#| classes: custom2
#| output-location: "column"
safo |> 
...
-   We can also modify manually the parameters of the created flowchart using the `fc_modify()` function:
...
## Thank you! {background-color="lightgrey"}
